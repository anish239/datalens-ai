...
import { extractAndVerifyToken } from './auth/tokenVerification';
import { runAiAnalyst } from './ai/orchestrator';
...
export function createApp(): express.Express {
  ...
  app.post('/api/analyses', (req, res) => {
    try {
      const user = extractAndVerifyToken(req.headers.authorization);
      const record: AnalysisRecord = req.body;
      if (!record || !record.id) {
